...
def enterTheZone():
    ...
    #容错处理
    while(True):
        url = "http://www.huya.com/g/{}".format(label)
        #获取gid（gameid）
...
def saveToExcel(roomList,zoneName):
    ...
    worksheet.write(0,3,label = '观看人数',style = sty1)
    worksheet.write(0,4,label = '传送门',style = sty1)
    for dict in roomList:
        ...
        worksheet.write(row,column + 4,xlwt.Formula('HYPERLINK("{}";"{}")'.format(dict["roomUrl"],dict["roomUrl"])))    #超链接
        row += 1
...
if __name__ == '__main__':
    ...
    old = time.time()
    saveInformation(roomList,zone[1])
